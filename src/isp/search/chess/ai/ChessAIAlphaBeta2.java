...
public class ChessAIAlphaBeta2 extends ChessAI{
    ...
    public ChessAIAlphaBeta2(ChessGame chessGame, PieceColor pieceColor, Function<GameState, Double> evalFunction, int depth) {
    ...
    public void move(boolean withOutput) {
        ...
        for (Move legalMove : allLegalMoves) {
            ...
            currentValue = currentGameState.getTurnColor() == PieceColor.WHITE ?
                    min(clonedGameState, depth - 1,highestValue,lowestValue) :
                    max(clonedGameState, depth - 1,highestValue,lowestValue);
    ...
    public double max(GameState currentGameState, int depth, double alpha, double beta) {
        ...
        for (Move legalMove : allLegalMoves) {
            ...
            currentHighest = Math.max(currentHighest,min(clonedGameState, depth - 1,currentHighest,beta)) ;

            if (beta <= currentHighest) {
                break;
            }
        }
    ...
    public double min(GameState currentGameState, int depth, double alpha, double beta) {
        if (depth <= 0 || currentGameState.isGameFinished()) return evalFunction.apply(currentGameState);
        ...
        for (Move legalMove : allLegalMoves) {
            ...
            currentLowest = Math.min(currentLowest,max(clonedGameState, depth - 1,alpha,currentLowest));
